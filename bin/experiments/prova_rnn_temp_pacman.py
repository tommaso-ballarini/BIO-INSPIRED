...
CONFIG_FILE_NAME = "temp_rnn_neat_pacman_config.txt"
NUM_GENERATIONS = 500
MAX_STEPS = 5000
import gymnasium as gym
import numpy as np
# --- COEFFICIENTI DI FITNESS (TUNING FACILE) --- 💡 NUOVA SEZIONE
SURVIVAL_BONUS_COEFF = 0.01     # Premio per ogni step sopravvissuto
EXPLORATION_BONUS_COEFF = 0.0  # Premio per ogni nuovo stato RAM unico raggiunto
STUCK_PENALTY_COEFF = 0.0      # Penalità per immobilità prolungata
EARLY_DEATH_PENALTY = 1.0      # Moltiplicatore se l'agente muore in meno di 300 steps
MAX_STUCK_RAM_STEPS = 2000      # Soglia di steps per applicare la penalità STUCK
...
def calculate_fitness_with_shaping(agent_decision_function, env_name, max_steps):
    """
    Fitness Function Avanzata per RAM/RNN con Reward Shaping e Tracking Metriche.
    
    Args:
        agent_decision_function (function): Funzione che prende lo stato RAM (features) e restituisce l'azione.
        env_name (str): ID dell'ambiente ALE (es. "ALE/Pacman-v5").
        max_steps (int): Numero massimo di step per episodio.
        
    Returns:
        tuple: (fitness_totale, metrics_dict)
    """
    
    # Inizializza l'ambiente ALE/Gymnasium (senza wrapper)
    # Obs_type="ram" è fondamentale qui
    env = gym.make(env_name, obs_type="ram", frameskip=2, repeat_action_probability=0.0)
    ...
    # Inizializzazione per tracking: usa le variabili globali definite all'inizio
    stuck_counter = 0
    
    # Lo stato iniziale è l'array RAM
    prev_ram_state = tuple(observation) 
    
    while not done and steps < max_steps:
        
        # Normalizzazione RAM per l'input della rete (come fatto nel worker)
        features = observation / 255.0
        
        # --- ATTIVAZIONE RETE & STEP ---
        ...
        # Hash dello stato RAM corrente per il tracking
        ram_hash = tuple(observation)
        
        # --- CALCOLO FITNESS COMPONENTI ---
        
        # 1. Punteggio Base (Reward grezzo da ALE)
        fitness += reward
        metrics["score_grezzo"] += reward
        
        # 2. Bonus Sopravvivenza (Longevità)
        fitness += SURVIVAL_BONUS_COEFF 
        metrics["bonus_survival"] += SURVIVAL_BONUS_COEFF
        
        # 3. Bonus Esplorazione (Cambio di Stato RAM)
        if ram_hash not in metrics["stati_ram_unici"]:
            metrics["stati_ram_unici"].add(ram_hash)
            fitness += EXPLORATION_BONUS_COEFF 
            metrics["bonus_esplorazione"] += EXPLORATION_BONUS_COEFF
            stuck_counter = 0 
        
        # 4. Anti-Stuck (Penalità se RAM non cambia a lungo)
        if ram_hash == prev_ram_state:
            stuck_counter += 1
        else:
            stuck_counter = 0
            
        if stuck_counter > MAX_STUCK_RAM_STEPS:
            fitness -= STUCK_PENALTY_COEFF
            metrics["penalita_stuck"] -= STUCK_PENALTY_COEFF
    ...
    # --- Penalità Morte Precoce (Finale) ---
    if steps < 300:
        # Se EARLY_DEATH_PENALTY è 0.5, moltiplichiamo per (1 - 0.5)
        fitness *= (1.0 - EARLY_DEATH_PENALTY)
    
    # Restituisce la fitness finale e le metriche
