...
def find_latest_run_dir():
    ...
    # Cerca cartelle che iniziano con 'run_si_'
    run_folders = [f for f in RESULTS_DIR.iterdir() if f.is_dir() and f.name.startswith("run_si_")]
    
    if not run_folders:
        print(f"❌ Errore: Nessuna cartella 'run_si_' trovata.")
        sys.exit(1)
...
def run_simulation(agent_path):
    ...
    try:
        ...
        # Seed fisso per la visualizzazione per coerenza
        ...
        print("--- INIZIO PARTITA (Premi Ctrl+C per interrompere la run attuale) ---")
        while not done:
            try:
            ...
            time.sleep(0.03) # ~30 FPS per visualizzazione godibile
            
            done = terminated or truncated
            
            if steps % 60 == 0:
                # obs[3] = Danger Center, obs[11] = Targeting Rel X
...
def main():
    print(f"--- VISUALIZZAZIONE INTERESTING AGENTS (Space Invaders) ---")
    ...
    while True:
        ...
        if not agents:
            print("Nessun agente interessante trovato (> 10 pts).")
            break
