...
def get_action(observation):
    ...
    if danger_center > 0.4:  # Increased threshold to prioritize danger
    ...
    bullet_left = observation[1] < -0.1 or observation[5] > 0.1
    
    # Adjust firing threshold if bullets are present
    if bullet_left:
        firing_threshold += abs(target_x) * 0.1 * 0.7
