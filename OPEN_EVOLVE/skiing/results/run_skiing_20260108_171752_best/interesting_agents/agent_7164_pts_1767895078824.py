def get_action(observation):
    ...
    if nearest_threat_dist_y < 3 and nearest_threat_dx * player_x > 0 and threat_type == -1.0:
        ...
        elif nearest_threat_dx < 0:
            action = 1  # Turn right to avoid the left tree
    
    # Additional logic to prioritize alignment over avoidance with a lower threshold
    if nearest_threat_dist_y < 4 and abs(target_delta_x) > 0.2:
