def get_action(observation):
    ...
    if abs(target_delta_x) > 0.1:
        action = 1 if target_delta_x > 0 else 2  # steer towards the gate
    elif abs(target_delta_x) <= 0.1 and nearest_threat_dist_y < 5:
        if threat_type == -1:  # Tree
            action = 2 if nearest_threat_dx > 0 else 1  # steer away from trees
        elif threat_type == -0.5:  # Flag Pole
            action = 0 if target_delta_x >= 0 else (1 if nearest_threat_dist_y < 3 and nearest_threat_dx * threat_type < 0 else 2)
    else:
