...
# IMPORTANT: Config must have [NEAT] feed_forward = False and num_inputs = 22
DEFAULT_CONFIG = str(PROJECT_ROOT / "config" / "neat_freeway_config.txt")
DEFAULT_OUTDIR = PROJECT_ROOT / "results" / "neat_freeway_rnn_shaped_timepenalty"
...
def plot_results(stats, save_dir):
    ...
    if stats.most_fit_genomes:
        ...
        # The shaded area represents the 'intervals' (Standard Deviation)
        ...
        plt.title("NEAT RNN - Shaped Fitness + Time Penalty")
...
def eval_genomes_factory(env_id, max_steps, episodes_per_genome, seed_base):
    def eval_genomes(genomes, config):
        ...
        for genome_id, genome in genomes:
            # Use RecurrentNetwork for RNN genomes
            ...
            for ep in range(episodes_per_genome):
                ...
                for t in range(max_steps):
                    # RNN activation maintains internal state between calls
                ...
                # Shaped Fitness: Balance progress, speed, and safety
...
def main():
    ...
    # Training with fixed seed for reproducibility
